...
<table>
  <tbody>
      ...
      <tr>
          <td><b>Query description</b></td>
          <td>Select User Post Replies</td>
      </tr>
      <tr>
          <td><b>Query frequency</b></td>
          <td>tens per day</td>
...
WHERE Project.idProject = $idProject
AND Task.idProject = Project.idProject
AND NOT EXISTS (SELECT Assigned.idTask
              FROM Assigned
             WHERE Task.idTask = Assigned.idTask;
```
...
<table>
  <tbody>
    ...
    <tr>
      <td><b>Justification</b></td>
      <td>To improve performance of full text search while searching for usernames; GIN because usernames never changes and name changes are rare.</td>
    </tr>
  </tbody>
</table>

```SQL
CREATE INDEX search_username ON UserTable USING GIN (to_tsvector('english', username || name));
```
...
<table>
  <tbody>
    ...
    <tr>
      <td><b>Justification</b></td>
      <td>To improve performance of full text search while searching for project names; GIN because updates in the project name and description are rare.</td>
    </tr>
  </tbody>
</table>

```SQL
CREATE INDEX search_project_name ON Project USING GIN (to_tsvector('english', name || description));
```
...
<table>
  <tbody>
    ...
    <tr>
      <td><b>Related queries</b></td>
      <td>SELECT17, SELECT18, SELECT19</td>
    </tr>
    <tr>
      <td><b>Index relation</b></td>
      <td>Task</td>
    ...
    <tr>
      <td><b>Justification</b></td>
      <td>To improve performance of full text search while searching for tasks; GIN because the updates in the task title and description are rare.</td>
    </tr>
  </tbody>
</table>

```SQL
CREATE INDEX search_task_name ON Task USING GIN (to_tsvector('english', title || description));
```
...
1. Removed schema indication, opting by the use of SET SCHEMA instructions.
2. Improved Full text search indexes.

***

GROUP1743, 09/04/2018
